...
import json
fr = open("./demoall.json", encoding="utf-8")
file = json.load(fr)
fr.close()

for index, date in enumerate(datelist):
    allvalue = file[date]
    for province_name, province_date in allvalue.items():
        if index:

            province_date["province_"] = province_date[
                "province_confirmedCount"] - file[datelist[
                    index - 1]][province_name]["province_confirmedCount"]
            citys = province_date["citys"]
            for city_index, city_date in enumerate(citys):
                city_date[
                    "city_increase"] = city_date["city_confirmedCount"] - file[
                        datelist[index - 1]][province_name]["citys"][
                            city_index]["city_confirmedCount"]
        else:
            province_date["province_increase"] = 0
            citys = province_date["citys"]
            for city_index, city_date in enumerate(citys):
                city_date["city_increase"] = 0

jsObj = json.dumps(file, ensure_ascii=False)
with open("China_all_data.json", "w", encoding="utf-8") as fw:
    fw.write(jsObj)
    fw.close()
